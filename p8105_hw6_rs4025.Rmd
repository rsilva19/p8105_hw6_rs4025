...
Since we are starting off with 20 variables, we elimate some based on general hypotheses for variables that might not explain birthweight well. We also elimate weight variables for which there exists a BMI variables based on that category, accounting for possible multicolinearity. 
...
In order to predict birthweight, we want to understand how our given variables relate to birthweight, and ultimately, include the most important predictors of birthweight in our model. 

We include some graphical dispictions of the relationships that seem to have the strongest association. 
```{r, warning = FALSE}
# correalation matrix 
birthweight_numeric = 
...
# chose wtgain over delivery weight because thinking more relative? WHICH ONE 
birthweight %>% 
...
summary(subset)
leaps(x =hospital_subdata[,2:7], y =hospital_subdata[,1], nbest =2, method ="Cp")
